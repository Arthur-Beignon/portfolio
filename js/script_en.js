  ...
  `;
    ...
    ">
      ...
      <p style="margin: 0 0 1.75rem; color: #666; font-size: 0.9rem; line-height: 1.6;">
        ${isSuccess
            ? 'Your message has been successfully sent.<br>I\'ll get back to you as soon as possible.'
            : 'An error occurred while sending your message.<br>Please try again or reach me directly by email.'}
      </p>
        ...
        btn.addEventListener("click", () => {
            ...
            if (countEl) {
                countEl.innerHTML = filter === "all"
                    ? `<span>${visible}</span> projects total`
                    : `<span>${visible}</span> project${visible > 1 ? "s" : ""} in <strong>${btn.textContent.trim()}</strong>`;
            }
